...
| 0x22 | CMD_SET_TELEMETRY | 上位机→下位机 | 设置 F1/F2/F3 速率与开关；支持连续 F1 扩展 |
| 0x23 | CMD_CAPTURE_BURST | 上位机→下位机 | 触发一次 F4 16 kHz 原始波形抓取 |

### 0x22 连续 F1 扩展载荷

旧载荷保持 7 字节 `<BHHH>`：`flags, f1_ms, f2_ms, f3_ms`。新固件同时
接受 10 字节 `<BHHHHB>`，尾部增加：

| 偏移 | 类型 | 字段 | 说明 |
|---:|---|---|---|
| 7 | uint16 | f1_rate_hz | 1000/2000/4000/8000/16000；必须整除 16 kHz FOC 频率 |
| 9 | uint8 | f1_batch_samples | 1..32，推荐 16 |

扩展模式由 16 kHz FOC 中断将原始控制周期快照写入单生产者/单消费者环形缓冲，
主循环再批量封装为 0xF1。中断内不调用 lwIP 或发送函数。16 kHz、16 点/帧时，
数据是 16000 点/秒，但协议/TCP 帧率仍约 1000 帧/秒。连续多 kHz 模式仅由以太网
发送；旧 7 字节命令仍沿用 TCP 1 kHz / UART 200 Hz 的平均采样路径。
...
高速通道用于电角度、Iq、相电流与施加电压。旧模式为 200 Hz（UART/RS-485）或 1000 Hz（以太网 TCP）；扩展连续流在 TCP 上支持 1/2/4/8/16 kHz。载荷可包含 1 个或多个样本，按 `payload_length` 判定单样本字节数：
