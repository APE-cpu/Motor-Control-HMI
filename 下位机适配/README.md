...
> 本文主体描述现有v1。新项目建议先阅读[PROTOCOL_V2.md](PROTOCOL_V2.md)，
> v2参考实现为`v2_protocol_portable.h/.c`，上电联调必须按
> [V2_无功率级联调清单.md](V2_无功率级联调清单.md)执行。经典CAN暂不能直接承载v2。
...
| `0x13` | 故障复位 | 上位机→下位机 | 清除故障锁定（保护条件未清除时必须 NACK） |
| `0x20` | 设置参数 | 上位机→下位机 | 下发控制参数 |
| `0x21` | 设置传感器 | 上位机→下位机 | 配置位置传感器 |

> v2 协议在此基础上增加 0xF0/0xF1/0xF2 三种遥测通道，命令字含义与 v1 一致但承载在 v2 帧结构内，详见 [PROTOCOL_V2.md](PROTOCOL_V2.md)。
...
> v1 串口遥测 payload 为 **15 字节**（含 1 字节 padding），上位机解码格式 `<hHhHhbBBBx`。早期文档误写为 14 字节，以代码为准。
...
typedef struct {
    ...
    uint8_t  flags;             // 标志位
    uint8_t  padding;           // 填充字节（保持 15 字节对齐）
} TelemetryPayload_t;  // 总共15字节
#pragma pack(pop)
```
...
| `current_actual` | int16 | mA | 实际电流（需除100，如5.2A→520） |
| `angle_raw` | uint16 | - | 传感器原始值（Hall步数/QEP脉冲等） |
| `angle_actual` | int16 | 0.01° | 实际角度（需除100，如120.5°→12050） |
...
| `flags` | uint8 | 位掩码 | bit0=低速警告, bit1=过流, bit2=驱动故障, bit3=急停 |
| `padding` | uint8 | - | 填充字节（保持 15 字节对齐） |

**完整帧示例**（转速1500rpm，电流5.2A，角度120.5°，温度45°C）：
```
AA 01 0F F4 05 DC 05 08 02 D2 04 12 2F 55 FA FF 00 [校验和] 55
```

> 命令字 0x01 是 v1 常规遥测。v2 协议用命令字 0xF0/0xF1/0xF2 区分三种遥测通道，详见 [PROTOCOL_V2.md](PROTOCOL_V2.md)。
