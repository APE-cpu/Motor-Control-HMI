...
def test_high_rate_iq_drives_current_curve_and_csv(tmp_path):
    ...
    assert page._c_current._times.maxlen == 2000
    assert page._c_angle._times.maxlen == 2000
    assert list(page._c_current._buffers["实际 Iq"])[-2:] == [0.125, -0.25]
    assert list(page._c_current._buffers["给定 Iq"])[-2:] == [0.0, 0.0]
    assert list(page._c_current._buffers["Ia"])[-2:] == [0.2, -0.3]
    assert list(page._c_current._buffers["Ib"])[-2:] == [-0.1, 0.15]
