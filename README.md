...
│   ├── digital_twin_page.py      # 独立数字孪生工作台（运行、负载、机械参数、扰动）
...
- 电机级电流限幅随参数帧发送到真机并写入实验保护快照；数字孪生使用独立模型参数，不与固件控制器单位强行映射
...
- 定时刷新（默认 100 ms）
- “数字孪生”独立页面管理仿真环境、模型运行、目标转速、机械参数和负载扰动；监控页只观察公共遥测
- 启动仿真环境只建立静止数据源，点击“运行模型”后才旋转；真机连接期间不能启动仿真环境
...
数字孪生现在作为独立仿真工作台存在，不再假设与 F407 固件的控制结构、离散周期和参数单位一一对应。固件参数在“电机控制”页设置；仿真目标、机械参数和负载在“数字孪生”页设置。
...
- 独立工作台可启动/停止仿真环境与模型，设置目标转速、B/Tc/J、负载类型及周期/脉冲扰动
- 模型可产生阶跃、超调、滑行停机、齿槽转矩、库仑摩擦和铜损发热等趋势；结果不作为固件 PI 整定依据
...
**Q: 没有真实下位机能用吗？**
可以。进入“数字孪生”页，先启动仿真环境，再运行模型，即可生成模拟遥测并验证 UI 与训练流程。
