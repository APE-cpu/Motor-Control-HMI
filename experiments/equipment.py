...
def default_78w_equipment() -> EquipmentProfile:
    return EquipmentProfile(
        ...
        revision=1,
        name="78W PMSM 低压实验平台",
        motor_type="PMSM",
        rated_power_w=78.0,
        nominal_bus_voltage_v=48.0,
        inverter="两电平低压逆变器（待补型号）",
        controller="控制板（待补型号与设备ID）",
        sensors=["增量式编码器(QEP)"],
        # 尚未实测的控制板身份不能编造；用户拿到板后创建新修订补齐。
        expected_device_id="",
        safety_limits={
            "max_rpm": 3000.0, "max_bus_voltage_v": 60.0,
            "max_current_a": 5.0, "max_temperature_c": 80.0,
        },
        notes="内置起点；连接真实控制板前必须建立含设备ID的新修订。",
        created_at=_iso_now(),
        built_in=True,
    )
